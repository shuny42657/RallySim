...
def train_high_level(env, env_test, high_level_agent,low_level_agent, args):
    ...
    while total_step_cnt in range( int(args['total_step_num']) ):
        ...
        low_level_states = {}
        low_level_actions = {}
        high_level_flags = {agent : False for agent in env.agents} 
        ...
        while epoch_step < int(args['steps_per_epoch']):
            ...
            if terminal or epoch_ended:
                replay_buffer.store(state_buffer,action_buffer,0.0,value_buffer,logp_buffer)
                epoch_step += 1
