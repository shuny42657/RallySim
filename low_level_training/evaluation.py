...
def evaluate_greedy(env_test, agent, args, test_iter, test_n, state_dim, latent_cont_dim, latent_disc_dim):
    ...
    while True:
        ...
        z = np.array([[-0.38727294, -0.55608423]])
        ...
        for t_test in range(int(args['max_episode_len'])):
            ...
            action_test = np.clip(action_test,-1.0,1.0)
        ...
        return_epi_test = 0                                                              ##int(return_epi_test)))
...
def main(args):
    print('load env')
    unity_env = UnityEnvironment(args["env"],worker_id=int(args['port_offset']))
    print('wrap env')
    test_env = UnityToGymWrapper(unity_env)
    record_env = RecordVideo(test_env,'./videos')
    ...
    return_test_epi = evaluate_greedy_ppo(record_env,agent,args,0,0,state_dim)
